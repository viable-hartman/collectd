        ...
        derive_t io_syscw)
{
    ...
    dispatch_value_helper(&vl, "ps_count", 2, doing_detail, config->ps_count);

    vl.values[0].gauge = vmem_size;
    dispatch_value_helper(&vl, "ps_vm", 1, doing_detail, config->ps_vm);

    vl.values[0].gauge = vmem_rss;
    dispatch_value_helper(&vl, "ps_rss", 1, doing_detail, config->ps_rss);

    vl.values[0].gauge = vmem_data;
    dispatch_value_helper(&vl, "ps_data", 1, doing_detail, config->ps_data);

    vl.values[0].gauge = vmem_code;
    dispatch_value_helper(&vl, "ps_code", 1, doing_detail, config->ps_code);

    vl.values[0].gauge = stack_size;
    dispatch_value_helper(&vl, "ps_stacksize", 1, doing_detail,
                          config->ps_stacksize);

    vl.values[0].derive = vmem_minflt_counter;
    vl.values[1].derive = vmem_majflt_counter;
    dispatch_value_helper(&vl, "ps_pagefaults", 2, doing_detail,
                          config->ps_pagefaults);

    vl.values[0].derive = cpu_user_counter;
    vl.values[1].derive = cpu_system_counter;
    dispatch_value_helper(&vl, "ps_cputime", 2, doing_detail,
                          config->ps_cputime);
    ...
    if ( (io_rchar != -1) && (io_wchar != -1) )
    {
        ...
        dispatch_value_helper(&vl, "ps_disk_octets", 2, doing_detail,
    ...
    if ( (io_syscr != -1) && (io_syscw != -1) )
    {
        ...
        dispatch_value_helper(&vl, "ps_disk_ops", 2, doing_detail,
                              config->ps_disk_ops);
    }
