...
<<<<<<< HEAD
<<<<<<< HEAD
The C<write_gcm> plugin writes metrics to I<Stackdriver Monitoring>. I<Google
Stackdriver> provides monitoring, logging, and diagnostics for applications in
the cloud. GCM stands for I<Google Cloud Monitoring>.
...
I<metadata server> and used to authenticate to Stackdriver.
...
The C<write_gcm> plugin writes metrics to the I<Google Cloud Monitoring> (GCM)
service.
>>>>>>> Remove erroneous doc indentation that caused bad formatting.
=======
The C<write_gcm> plugin writes metrics to I<Stackdriver Monitoring>. I<Google
Stackdriver> provides monitoring, logging, and diagnostics for applications in
the cloud. GCM stands for I<Google Cloud Monitoring>.
>>>>>>> Added write_gcm to README file. (#104)
...
The C<write_gcm> plugin writes metrics to the
...
minimum. The I<write_gcm plugin> requires the
